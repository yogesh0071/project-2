...
# ---------------------------------------------------------------------------
# SECURITY GROUPS - least privilege: SSH only from your IP, app ports open
# ---------------------------------------------------------------------------

resource "aws_security_group" "app_server" {
  name        = "${var.project_name}-app-sg"
  description = "Security group for app/k3s server"
  vpc_id      = aws_vpc.main.id
  ...
  ingress {
    description = "k3s API - needed for Ansible/kubectl from your machine"
    from_port   = 6443
    to_port     = 6443
    protocol    = "tcp"
    cidr_blocks = [var.my_ip]
...
resource "aws_security_group" "jenkins_server" {
  ...
  ingress {
    description = "Jenkins web UI - restricted to your IP, not the whole internet"
    from_port   = 8080
    to_port     = 8080
    protocol    = "tcp"
    cidr_blocks = [var.my_ip]
...
# ---------------------------------------------------------------------------
# IAM - least privilege role so the app server can push/pull from ECR
# ---------------------------------------------------------------------------
...
resource "aws_iam_instance_profile" "jenkins_ecr_profile" {
  name = "${var.project_name}-jenkins-ecr-profile"
  role = aws_iam_role.jenkins_ecr_role.name
}
